...
def parse_args():
    ...
    parser.add_argument("--current-task", default=0, type=int, help='task identity')
    parser.add_argument(
        "--deepspeed",
        type=str,
        default=None,
...
def main():
    ...
    ############################################
    cfg.train_dataset = cfg.train_dataset[args.current_task]
    cfg.train_dataloader['dataset'] = cfg.train_dataset
    
    visualizer = dict(
        type='mmengine.visualization.Visualizer',
        vis_backends=[dict(
                type='swanlab.integration.mmengine.SwanlabVisBackend',
                init_kwargs=dict(project='cvit', experiment_name=f'{osp.basename(args.config)}/task{args.current_task}')
            )]
            )
    ...
    if cfg.get("framework", "mmengine").lower() == "huggingface":
        ...
        # let runner know the task-id of current task
        runner.cur_task = args.current_task
